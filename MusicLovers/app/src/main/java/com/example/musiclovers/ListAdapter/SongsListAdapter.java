...
public class SongsListAdapter extends RecyclerView.Adapter<SongsListAdapter.ViewHolder>{
    ...
    private final ArrayList<Song> songItems;
    private Context context;

    String base_Url = "http://10.0.2.2:3000/";
    public Fragment base;

    public SongsListAdapter(int layoutHolder, int tvName, int tvArtist, int ivImage, ArrayList<Song> songItems, int clickMode, Context context) {
        ...
        holder.itemView.setOnLongClickListener(view -> {
            if(clickMode == 1){
                removeFromPlaylist(
                        ...
                        ((PlaylistDetail) base).playlist.get_id()
                );
                ...
                popup.setOnMenuItemClickListener(item -> {
                    if(item.getItemId() == R.id.song_opt_add_to_playlist){
                        ...
                        ((PlayingNext) MainActivity.playingNext).nextSongs.add(songItems.get(position));
                        ((PlayingNext) MainActivity.playingNext).adapter.notifyDataSetChanged();
