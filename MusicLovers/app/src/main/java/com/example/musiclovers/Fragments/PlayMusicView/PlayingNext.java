 ...
 * DONE
 */
public class PlayingNext extends Fragment implements MainActivity.UpdateFragmentPlayingNext {
    RecyclerView recyclerView;
    ImageButton btnShuffle;
    ImageButton btnRepeat;
    TextView tvNoSongPlayingNext;
    public ArrayList<Song> nextSongs = new ArrayList<>();
    boolean isShuffle;
    ArrayList<Song> tempSongArray = new ArrayList<>();
    ...
    @Override
    public void initDisplayNextSongsList(ArrayList<Song> songsList, int position) {
        if(isShuffle){
            tempSongArray.clear();
            tempSongArray.addAll(((MainActivity) getContext()).songList);
    ...
    @Override
    public void updateDisplayNextSongsList(Boolean isNext, Boolean isPrevious, Song song) {
        if(!nextSongs.isEmpty()){
            if(isNext){
                nextSongs.remove(0);
            } else if(isPrevious){
